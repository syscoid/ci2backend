...
class Breadcrumbs
{
    ...
    function show()
    {
        ...
        if ($this->breadcrumbs) {
        	...
        	$this->breadcrumbs = unique_multidim_array($this->breadcrumbs, 'page');

            // set output variable
            $output = $this->tag_open;
            
            $first = array_values(array_keys($this->breadcrumbs));
            
            $end_bre = '<span class="divider-last">&nbsp;</span>';
            ...
            foreach ($this->breadcrumbs as $key => $crumb) {
                ...
                if ($breadcrumbs == $key) {
                    ...
                    $output .= $this->crumb_open . '<i class="icon-home"></i><a href="' . $crumb['href'] . '">' . $crumb['page'] . '</a>' . $this->crumb_close;
